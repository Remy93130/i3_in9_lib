 ...
 * @param i        L'index.
 */
void _swap(p_s_vector_v2_double p_vector, size_t i)
{
    double index_value = get(p_vector, i);
    vector_v2_double_set(p_vector, i, get(p_vector, i + 1));
    vector_v2_double_set(p_vector, i + 1, index_value);
}
 ...
 */
void _sort(p_s_vector_v2_double p_vector, size_t list_size)
{
...
void insert_erase_random(p_s_vector_v2_double p_vector, size_t n)
{
    for (size_t i = 0; i < n; ++i)
    {
        ...
        vector_v2_double_insert(p_vector, rng, -1);
    }
    for (size_t i = 0; i < n; ++i)
    {
        size_t rng = random_size_t(0, p_vector->size);
        vector_v2_double_erase(p_vector, rng);
    }
}
...
{
    for (size_t i = 0; i < n; ++i)
        vector_v2_double_insert(p_vector, 0, -1);
    for (size_t i = 0; i < n; ++i)
...
{
    for (size_t i = 0; i < n; ++i)
        vector_v2_double_push_back(p_vector, -1);
    for (size_t i = 0; i < n; ++i)
        vector_v2_double_pop_back(p_vector);
}
...
void read_write_random(p_s_vector_v2_double p_vector, size_t n)
{
    ...
    {
        size_t rng = random_size_t(0, p_vector->size);
        vector_v2_double_set(p_vector, rng, -1);
    }
    for (size_t i = 0; i < n; ++i)
    {
        size_t rng = random_size_t(0, p_vector->size);
...
void read_write_sequential(p_s_vector_v2_double p_vector, size_t n)
{
    for (size_t i = 0; i < n; ++i)
    {
        for (size_t j = 0; j < p_vector->size; ++j)
            vector_v2_double_set(p_vector, j, j);
        for (size_t j = 0; j < p_vector->size; ++j)
            get(p_vector, j);
    }
}
...
void bubble_sort(p_s_vector_v2_double p_vector, size_t n)
{
    for (size_t i = 0; i < n; ++i)
    {
        for (size_t j = 0; j < p_vector->size; ++j)
        {
            ...
            vector_v2_double_set(p_vector, j, rng);
        }
...
int main(int argc, char const *argv[])
{
    ...
    p_s_vector_v2_double vector = vector_v2_double_alloc(init_size);
    for (int i = 0; i < init_size; ++i)
        vector_v2_double_set(vector, i, random_double(0, 100));
